  ...
  c_csplayer* const shooter, const weapon_info_t* const wpn_data, const c_game_trace& enter_trace, vector3d& src, const vector3d& dir, int& pen_count, float& cur_dmg, const float pen_modifier )
{
  ...
  // NOTE: squared is CORRECT for 2018 CS:GO (verified: qo0 IDA of
  // CCSPlayer::HandleBulletPenetration, nixware, ayyware - all use LengthSqr/24).
  // a Length() "fix" here overestimates wallbang damage and shoots walls.
...
void c_auto_wall::scale_dmg( c_csplayer* const player, float& dmg, const float armor_ratio, const int hitgroup )
{
  const auto has_heavy_armor = player->has_heavy_armor( );
  const bool victim_ct = player->team( ) == 3;

  switch( hitgroup )
  {
  case 1:
  {
    // server: 4x base, then per-team gamerules scale (default 1.0, matters on
    // modified modes). previously hardcoded 4.f with a garbage crosshair param.
    dmg *= 4.f;

    auto head_scale = victim_ct ? cvars::mp_damage_scale_ct_head : cvars::mp_damage_scale_t_head;
    if( head_scale )
      dmg *= head_scale->get_float( );
  ...
  case 3:
  {
    auto body_scale = victim_ct ? cvars::mp_damage_scale_ct_body : cvars::mp_damage_scale_t_body;
    if( body_scale )
      dmg *= body_scale->get_float( );
    ...
    if( target )
    {
      if( trace.entity )
      {
        ...
        if( ( trace.entity == target || ( is_player && ( ( c_csplayer* )trace.entity )->team( ) != shooter->team( ) ) ) && ( ( trace.hitgroup >= 1 && trace.hitgroup <= 7 ) || trace.hitgroup == 10 ) )
        {
          ...
          this->scale_dmg( data.hit_player, cur_dmg, wpn_data->armor_ratio, data.hitgroup );
