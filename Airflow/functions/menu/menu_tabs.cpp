    ...
    case 0:
    {
      ...
      ImGui::SetColumnOffset( 1, 270 );
      {
        ...
        begin_child( xor_c( "General" ) )
        {
          ...
          this->combo( xor_c( "Fakelag correction" ), &g_cfg.rage.fakelag_correction, fakelag_correction, IM_ARRAYSIZE( fakelag_correction ) );
          this->checkbox( xor_c( "Delay shot on unduck" ), &g_cfg.rage.delay_unduck );
          this->slider_int( xor_c( "Auto bodyaim after X miss" ), &g_cfg.rage.auto_baim_misses, 0, 10, xor_c( "%d" ) );
    ...
    case 1:
    {
      ...
      ImGui::SetColumnOffset( 1, 270 );
      {
        ...
        begin_child( xor_c( "Fake angle" ) )
        {
          ...
          this->combo( xor_c( "Fake yaw" ), &g_cfg.antihit.fake_type, aa_fake_type, IM_ARRAYSIZE( aa_fake_type ) );
          if( g_cfg.antihit.fake_type == 1 )
          {
            this->slider_int( xor_c( "Jitter min##fake" ), &g_cfg.antihit.fake_jitter_min, 0, 180, xor_c( "%d" ) );
            this->slider_int( xor_c( "Jitter max##fake" ), &g_cfg.antihit.fake_jitter_max, 0, 180, xor_c( "%d" ) );
          }
          if( g_cfg.antihit.fake_type == 2 )
            this->slider_int( xor_c( "Spin speed##fake" ), &g_cfg.antihit.fake_spin_speed, 10, 300, xor_c( "%d" ) );
          this->slider_int( xor_c( "LBY delta##desync" ), &g_cfg.antihit.desync_range, 0, 180, xor_c( "%d" ) );
          this->combo( xor_c( "Freestand mode" ), &g_cfg.antihit.freestanding_mode, aa_freestand_mode, IM_ARRAYSIZE( aa_freestand_mode ) );
        }
        end_child;
      }
      ImGui::NextColumn( );
      {
        begin_child( xor_c( "Fake lag" ) )
        {
          ...
          if( g_cfg.antihit.fakelag )
            this->checkbox( xor_c( "Break lagcomp" ), &g_cfg.antihit.fakelag_break_lc );

          if( g_cfg.antihit.fakelag )
            this->checkbox( xor_c( "Jitter limit" ), &g_cfg.antihit.fakelag_jitter );
        }
        end_child;
