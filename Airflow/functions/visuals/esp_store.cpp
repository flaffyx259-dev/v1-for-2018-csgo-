...
void c_esp_store::store_players( )
{
  ...
  for( auto& entity : entities )
  {
    ...
    else
    {
      ...
      info.last_seen_time = interfaces::global_vars->real_time;
    ...
    // player resource snapshot works while dormant too, keep ping fresh always
    if( interfaces::player_resource )
      flags.ping = interfaces::player_resource->ping( player );
