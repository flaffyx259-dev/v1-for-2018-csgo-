...
if( enemy_esp.elements & 2 )
  ...
  if( enemy_esp.elements & 8 && !info->dormant )
  ...
  if( enemy_esp.elements & 16 && !info->dormant )
  ...
  if( enemy_esp.elements & 32 && !info->miscweapon && !info->dormant )
  ...
  if( enemy_esp.elements & 64 )
  {
    ...
    // ping from player resource (fresh even while dormant), color-coded
    if( flags.ping > 0 )
    {
      color ping_clr = flags.ping < 60 ? color( 162, 255, 115 ) : ( flags.ping < 120 ? color( 255, 220, 100 ) : color( 255, 95, 95 ) );
      info->objs [ iter++ ] = esp_objects_t{ true, esp_right, ping_clr, 0.f, 0.f, std::to_string( flags.ping ) + xor_str( "ms" ), font_pixel, false, info->dormant_alpha };
    }

    // dormant tag with seconds since last seen (sound esp may be old)
    if( info->dormant )
    {
      int secs = info->last_seen_time > 0.f ? ( int )( interfaces::global_vars->real_time - info->last_seen_time ) : 0;
      if( secs < 0 )
        secs = 0;
      info->objs [ iter++ ] = esp_objects_t{ true, esp_right, color( 150, 150, 150 ), 0.f, 0.f, xor_str( "DORM " ) + std::to_string( secs ) + xor_str( "s" ), font_pixel, false, info->dormant_alpha };
    }
