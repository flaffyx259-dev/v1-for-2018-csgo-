...
struct configs_t
{
  struct ragebot_t
  {
    ...
    bool delay_shot{ };
    bool delay_unduck{ };
    int fakelag_correction{ }; // 0 off, 1 delay shot, 2 refine shot, 3 delay + refine
    int auto_baim_misses = 3; // force bodyaim after this many misses (0 = off)
    bool anti_mrx{ };
    bool adaptive_spike{ };
    int spike_amt{ };
  ...
  struct anti_hit_t
  {
    ...
    bool desync{ };
    int desync_type{ }; // lby breaker: 0 disabled, 1 static, 2 jitter, 3 spin, 4 random, 5 opposite
    int desync_range = 90;

    int fake_type{ }; // fake yaw: 0 opposite, 1 jitter, 2 spin, 3 random (all opposite-anchored)
    int fake_spin_speed = 120; // spin oscillation speed around opposite (10..300)
    int fake_jitter_min = 15; // jitter magnitude band (randomized every send)
    int fake_jitter_max = 45;
    int freestanding_mode{ }; // 0 damage+wall, 1 wall only, 2 damage only

    bool fakelag{ };
    bool fakelag_break_lc{ };
    bool fakelag_jitter{ };
