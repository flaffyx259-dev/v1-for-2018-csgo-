...
int c_fake_lag::get_choke_amount( )
{
  ...
  // jitter fakelag: alternate between full limit and ~half. a perfectly periodic
  // choke cycle lets them time shots to your update tick; alternating cycle
  // lengths makes update arrival unpredictable. flipped every sent packet.
  if( g_cfg.antihit.fakelag && g_cfg.antihit.fakelag_jitter && max_choke > 2
    && !g_cfg.binds [ dt_b ].toggled && !g_cfg.binds [ hs_b ].toggled )
  {
    if( !jitter_flip )
      max_choke = std::max< int >( 1, max_choke - ( max_choke / 2 ) );
  }

  // break lagcomp (2018 anti-resolver tech): choke enough ticks that every update
  // jumps the teleport distance (>64u = 4096 sq 2d, sv_lagcompensation_teleport_dist).
  // the enemy server walks history from current origin and drops everything past the
  // first >64u jump -> their backtrack dies, they must shoot our real-time pos
  // (which desync still protects). opportunistic: needs speed (ground run or fast
  // air), never below the user's limit, never above 16, never with dt/hs shifting.
  if( g_cfg.antihit.fakelag && g_cfg.antihit.fakelag_break_lc
    && !g_cfg.binds [ dt_b ].toggled && !g_cfg.binds [ hs_b ].toggled
    && max_choke > 1 )
  {
    ...
    if( speed > 10.f )
    {
      float dist = speed * interfaces::global_vars->interval_per_tick;
      if( dist > 0.01f )
      {
        int need = std::clamp( ( int )std::ceil( 65.f / dist ), 1, 16 );
        if( need > max_choke )
          max_choke = need;
      }
    }
...
{
  // jitter fakelag flips its cycle length every sent packet (see get_choke_amount)
  if( *g_ctx.send_packet )
